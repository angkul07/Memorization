...
def minimize_prompt(model, tokenizer, input_str, target_str, system_prompt, chat_template, device, optimization_args,
    ...
    while not done:
        ...
        input_ids, free_token_slice, input_slice, target_slice, loss_slice = utils.prep_text(input_str,
                                                                                                  target_str,
                                                                                                  tokenizer,
                                                                                                  system_prompt,
                                                                                                  chat_template,
        ...
        if optimization_args["discrete_optimizer"] == "gcg":
            solution = gcg.optimize_gcg(model, input_ids, input_slice, free_token_slice, target_slice,
            ...
            solution = random_search.optimize_random_search(model, input_ids, input_slice, free_token_slice,
        ...
        target_acquired = utils.check_output_with_hard_tokens(model, solution["input_ids"].unsqueeze(0),
                                                                   target_slice,
                                                                   loss_slice)
