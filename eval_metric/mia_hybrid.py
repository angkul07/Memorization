...
def specail_token_id(tokenizer):
    # Directly get token IDs by encoding the strings.
    yes_token = tokenizer.encode("Yes", add_special_tokens=False)[-1]
    no_token = tokenizer.encode("No", add_special_tokens=False)[-1]
    assistant_token = tokenizer.encode("Assistant:", add_special_tokens=False)[-1]
    # For 'ass_last', you can choose a default, for example encoding a single space.
    ass_last_token = tokenizer.encode(" ", add_special_tokens=False)[-1]
    
    token_id_dict = {
        "Yes": yes_token,
        "No": no_token,
        ":": assistant_token,
        "ass_last": ass_last_token,
    }
...
def main():
    ...
    parser.add_argument("--max_train_samples", type=int, default=1116) # Default value is the max dataset size of WikiMia-24 with 64 block size
